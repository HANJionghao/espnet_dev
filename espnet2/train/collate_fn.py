...
class HuBERTCollateFn(CommonCollateFn):
    ...
    def _add_noise_wavlm(self, data, speech, speech_id):
        ...
        if self.dynamic_mixing_prob >= np.random.random() or len(data) == 1:
            ...
            noise_db = np.random.uniform(-self.dynamic_mixing_gain_db, self.dynamic_mixing_gain_db)
        ...
        noise = noise * scale
        speech[speech_start : speech_start + length] += noise[noise_start : noise_start + length]
